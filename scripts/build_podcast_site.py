...
def build_index_html(eps: list[dict]) -> str:
    ...
    for e in eps:
        play_block = ""
        if e["has_audio"]:
            # Audio served from GitHub Pages (local relative path, no redirects)
            # IMPORTANT: do NOT use GitHub Release URLs — they 302 redirect to
            # Azure Blob with a 1-hour signed token, breaking mobile/podcast apps.
            audio_url = f"{e['slug']}/audio/podcast.mp3"
            play_block = f"""
